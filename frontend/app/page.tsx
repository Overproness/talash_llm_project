...
export default function HomePage() {
  ...
  const addFiles = useCallback((incoming: FileList | null) => {
    ...
    setFiles(prev => [
      ...prev,
      ...pdfs.map(f => ({ file: f, status: 'idle' as UploadStatus, message: '' })),
    ])
  }, [])
  ...
  const uploadAll = async () => {
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      setFiles(prev =>
        prev.map(f => f.file.name === item.file.name ? { ...f, status: 'uploading', message: 'Uploading…' } : f)
      )
      try {
        setFiles(prev =>
          prev.map(f => f.file.name === item.file.name ? { ...f, status: 'parsing', message: 'Parsing CV with AI…' } : f)
        )
        const res = await api.uploadCV(item.file)
        setFiles(prev =>
          prev.map(f =>
            f.file.name === item.file.name
              ? { ...f, status: 'done', message: res.message, candidateId: res.candidate_id }
              : f
          )
        )
      } catch (err: unknown) {
        ...
        setFiles(prev =>
          prev.map(f =>
            f.file.name === item.file.name ? { ...f, status: 'error', message: errorMessage } : f
          )
        )
      }
    }
    ...
    <div className="flex min-h-screen bg-surface">
      ...
      <main className="ml-[220px] flex-1">
        ...
        <div className="pt-24 px-8 pb-12">
          {/* Header */}
          <div className="mb-10">
            <h1 className="text-3xl font-extrabold tracking-tight text-on-surface">Upload Candidate CVs</h1>
            <p className="text-on-surface-variant mt-1 text-sm">Drop PDF files or click to select. The AI engine will parse and extract structured data.</p>
          </div>

          {/* Stats bar */}
          <div className="grid grid-cols-4 gap-6 mb-10">
            {[
              { icon: 'folder_zip', label: 'Queued', value: files.length, color: 'bg-primary-fixed text-on-primary-fixed' },
              { icon: 'sync', label: 'Processing', value: files.filter(f => f.status === 'uploading' || f.status === 'parsing').length, color: 'bg-tertiary-fixed text-on-tertiary-fixed' },
              { icon: 'check_circle', label: 'Completed', value: files.filter(f => f.status === 'done').length, color: 'bg-primary-fixed text-on-primary-fixed' },
              { icon: 'error', label: 'Failed', value: files.filter(f => f.status === 'error').length, color: 'bg-error-container text-on-error-container' },
            ].map(stat => (
              <div key={stat.label} className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm">
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-on-surface mb-1">Drop PDFs here or click to browse</h3>
            <p className="text-on-surface-variant text-sm">Supports PDF files up to 50 MB each</p>
            <input
              ref={fileInputRef}
              type="file"
              accept=".pdf"
          ...
          {files.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-4">Files</h2>
              {files.map(item => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-on-surface text-sm">{item.file.name}</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {item.status === 'idle' && (
                        <button onClick={() => removeFile(item.file.name)} className="text-outline hover:text-error transition-colors">
