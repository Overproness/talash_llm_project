      ...
      <main className="ml-[220px] flex-1">
        ...
        <div className="pt-24 px-8 pb-12">
          <div className="mb-8">
            ...
            <p className="text-on-surface-variant mt-1 text-sm">System configuration and status</p>
          </div>

          <div className="space-y-6 max-w-2xl">
            {/* System Status */}
            <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm">
              <h2 className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-5">System Status</h2>
              <div className="space-y-4">
                {[
                  { label: 'API Backend', value: health?.status === 'ok' ? 'Connected' : 'Checking…', ok: health?.status === 'ok' },
                  { label: 'Ollama LLM', value: health?.ollama === 'available' ? 'Online (llama3.2)' : 'Offline — rule-based fallback active', ok: health?.ollama === 'available' },
            ...
            {/* Configuration */}
            <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm">
              <h2 className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-5">LLM Configuration</h2>
              <div className="space-y-4">
                <div>
                  <label className="text-xs text-on-surface-variant uppercase tracking-widest font-bold block mb-2">Ollama Host</label>
                  <input
            ...
            <div className="bg-surface-container rounded-2xl p-6 border-l-4 border-primary">
              ...
              <ol className="space-y-2 text-sm text-on-surface-variant list-decimal list-inside">
                <li>Download Ollama: <span className="text-primary font-mono">winget install Ollama.Ollama</span></li>
                <li>Pull the model: <span className="text-primary font-mono">ollama pull llama3.2:3b</span></li>
                <li>Start Ollama: <span className="text-primary font-mono">ollama serve</span></li>
                <li>Refresh this page to verify connection</li>
              </ol>
              <p className="text-xs text-outline mt-3">When offline, the system uses regex-based CV extraction as fallback.</p>
            </div>
          </div>
        </div>
      </main>
