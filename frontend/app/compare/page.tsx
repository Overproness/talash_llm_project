...
const CANDIDATE_COLORS = [
  ...
  { bar: "bg-violet-500", text: "text-violet-600", border: "border-violet-500" },
];

function ScoreBar({ score, colorClass }: { score: number | null | undefined; colorClass: string }) {
  const pct = score ?? 0;
  return (
...
function SkillBadge({ skill, highlight }: { skill: string; highlight?: boolean }) {
  return (
    <span
...
export default function ComparePage() {
  ...
  const allSkills = details.length > 0
    ? Array.from(new Set(details.flatMap((c) => c.skills)))
    : [];

  // Shared skills = appear in every candidate
  const sharedSkills = allSkills.filter((s) => details.every((c) => c.skills.includes(s)));

  // Get highest education level label from a candidate
  const highestEdu = (c: CandidateFull) => {
    const order = ["phd", "pg", "masters", "ms", "mphil", "ug", "bachelors", "bs", "be", "hssc", "sse", "intermediate", "matric"];
  ...
  const totalExpYears = (c: CandidateFull) => {
    ...
    return yrs != null ? `${yrs.toFixed(1)} yrs` : `${c.experience.length} records`;
  };
      ...
      <main className="ml-[220px] flex-1">
        ...
        <div className="pt-24 px-8 pb-12">
          ...
          {details.length === 0 ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-surface-container">
                      ...
                      <th className="px-4 py-3 text-center text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Edu Level</th>
                      ...
                      <th className="px-4 py-3 text-center text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Missing</th>
                      ...
                      .map((c) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-center text-sm">
                            <span className={c.missing_fields_count > 0 ? "text-error font-semibold" : "text-emerald-600 font-semibold"}>
            ...
            <>
              <button
                onClick={() => { setDetails([]); setSelected([]); }}
              ...
              <section className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm mb-6">
                <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-5">Score Comparison</h2>
                <div className="space-y-5">
                  {(["Overall Score", "Education Score", "Experience Score"] as const).map((label) => (
                    <div key={label}>
                      <p className="text-xs text-on-surface-variant mb-2 font-semibold">{label}</p>
                      <div className="space-y-2">
                        {details.map((c, i) => {
                          const scoreMap = {
                            "Overall Score": c.overall_score,
                            "Education Score": c.education_analysis?.education_score,
                            "Experience Score": c.experience_analysis?.experience_score,
                          };
                          return (
                            <div key={c.id} className="flex items-center gap-3">
                              <span className={`text-[10px] font-bold w-28 truncate ${CANDIDATE_COLORS[i].text}`}>
                                {c.personal_info.name || c.filename}
                              </span>
                              <ScoreBar score={scoreMap[label]} colorClass={CANDIDATE_COLORS[i].bar} />
                            </div>
                          );
                        })}
                      </div>
                ...
                {details.map((c, i) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <div className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-xl mx-auto mb-3 bg-primary-fixed text-on-primary-fixed`}>
                        {(c.personal_info.name || "U").charAt(0).toUpperCase()}
                      </div>
                      <h3 className="font-bold text-on-surface text-base">{c.personal_info.name || c.filename}</h3>
                    ...
                    <div className="space-y-2 text-sm divide-y divide-outline-variant/20">
                      {[
                        ...
                        { label: "Career Trajectory", value: c.experience_analysis?.career_trajectory ?? "—" },
                      ].map(({ label, value }) => (
                        <div key={label} className="flex justify-between py-1.5 first:pt-0">
                          <span className="text-on-surface-variant">{label}</span>
                          <span className="font-semibold text-on-surface capitalize">{value}</span>
                        </div>
                      ))}
                      <div className="flex justify-between py-1.5">
                        <span className="text-on-surface-variant">Missing Fields</span>
                        <span className={c.missing_fields.length > 0 ? "font-semibold text-error" : "font-semibold text-emerald-600"}>
                          {c.missing_fields.length === 0 ? "✓ Complete" : `${c.missing_fields.length} missing`}
                        </span>
                      </div>
                    </div>
                    ...
                    {c.education.length > 0 && (
                      <div>
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-2">Education</p>
                        <ul className="space-y-1.5">
                          {c.education.slice(0, 3).map((e, idx) => (
                            <li key={idx} className="text-xs text-on-surface">
                              ...
                              <span className="text-on-surface-variant block truncate">{e.institution}</span>
                            </li>
                          ))}
                          {c.education.length > 3 && (
                            <li className="text-xs text-on-surface-variant">+{c.education.length - 3} more</li>
                          )}
                        </ul>
                      </div>
                    )}
                    ...
                    {(c.education_analysis?.education_gaps ?? []).length > 0 && (
                      <div>
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Education Gaps</p>
                        <ul className="space-y-1">
                          {c.education_analysis!.education_gaps.slice(0, 2).map((g, idx) => (
                            <li key={idx} className="text-xs text-amber-700 bg-amber-50 rounded px-2 py-1">
                              {g.justification
                    ...
                    {c.missing_fields.length > 0 && (
                      <div>
                        <p className="text-[10px] font-bold text-error uppercase tracking-widest mb-1">Missing Info</p>
                        <ul className="flex flex-wrap gap-1">
                          {c.missing_fields.slice(0, 5).map((f) => (
                            <li key={f} className="text-[10px] bg-error/10 text-error px-2 py-0.5 rounded-full">{f}</li>
                          ))}
                          {c.missing_fields.length > 5 && (
                            <li className="text-[10px] text-on-surface-variant">+{c.missing_fields.length - 5}</li>
                          )}
                        </ul>
                      </div>
                    )}
                    ...
                    {c.summary && (
                      <div>
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Summary</p>
                        <p className="text-xs text-on-surface leading-relaxed line-clamp-4">{c.summary}</p>
                      </div>
                    )}
              ...
              {allSkills.length > 0 && (
                <section className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm mt-6">
                  <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Skills Comparison</h2>
                  ...
                  >
                    {details.map((c) => (
                      <div key={c.id}>
                        <p className="text-xs font-semibold text-on-surface mb-2 truncate">{c.personal_info.name || c.filename}</p>
                        <div className="flex flex-wrap gap-1">
                          {c.skills.slice(0, 20).map((s) => (
                            <SkillBadge key={s} skill={s} highlight={sharedSkills.includes(s)} />
                          ))}
                          {c.skills.length > 20 && (
                            <span className="text-[10px] text-on-surface-variant">+{c.skills.length - 20} more</span>
                          )}
                        </div>
                      </div>
                    ))}
